import numpy as np
'''
  Implementation of Backward Max Pool activation function
  Input 
        x   : 2m*2n || np.array
...
'''
def back_maxpool(x:np.array,y:np.array,dzdy:np.array):
    ...
    for y_col_idx in range(0,len(y[y_row_idx])):
      ...
      for x_i in range(0,stride):
        for y_i in range(0,stride):
          if (x[x_i + x_row_start_idx, y_i + x_col_start_idx] == curr_val):
            dzdx[x_i + x_row_start_idx, y_i + x_col_start_idx] = dzdy[y_row_idx, y_col_idx]            
