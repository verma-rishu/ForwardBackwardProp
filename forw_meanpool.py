import numpy as np
'''
  Implementation of Forward Mean Pool activation function
  Input 
        x   : 2m*2n || np.array
  Output
        y   : m*n   || np.array 
'''
def forw_meanpool(x:np.array):
  ...
  for row_idx in range(0,newHeight):
    for col_idx in range(0,newWidth):
      ...
      dzdx[row_idx, col_idx] = np.mean(sel_region)
